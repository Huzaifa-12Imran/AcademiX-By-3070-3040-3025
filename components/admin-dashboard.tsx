...
export default function AdminDashboard({ userName, onLogout }: AdminDashboardProps) {
  ...
  const [emailNotifications, setEmailNotifications] = useState<boolean>(() => {
    try {
      return localStorage.getItem("admin_email_notifications") === "true"
    } catch {
      return true
    }
  })
  ...
  const toggleEmailNotifications = () => {
    setEmailNotifications((v) => {
      const nv = !v
      try {
        localStorage.setItem("admin_email_notifications", nv ? "true" : "false")
      } catch {}
      return nv
    })
  }
  ...
  const toggleMaintenanceMode = () => {
    setMaintenanceMode((v) => {
      const nv = !v
      try {
        localStorage.setItem("admin_maintenance_mode", nv ? "true" : "false")
      } catch {}
      return nv
    })
  }
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Enable system email alerts for important events.</p>
                  </div>
                  <button
                    onClick={toggleEmailNotifications}
                    className={`px-3 py-1 rounded-lg font-medium ${emailNotifications ? "bg-primary text-primary-foreground" : "bg-input text-foreground"}`}
                  >
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Disable public access while performing maintenance.</p>
                  </div>
                  <button
                    onClick={toggleMaintenanceMode}
                    className={`px-3 py-1 rounded-lg font-medium ${maintenanceMode ? "bg-destructive text-destructive-foreground" : "bg-input text-foreground"}`}
                  >
                ...
                <div>
                  <p className="text-sm text-muted-foreground">Changes are stored locally in your browser for this demo.</p>
                </div>
              </div>
            </div>
          )}
